...
Every ITASC run is a chain of stages, and the chain is written to disk. Each
...
ITASC expects a project directory with **one subdirectory per position**
...
The full `itasc` app runs these in order. The name in parentheses is the
...
2. **Cellpose** (full `itasc` app). Run the nucleus and cell channels to
   ...
   segmentation.
3. **Nucleus tracking** (`itasc-tracking`). Build the Ultrack candidate
   database from the maps, solve the tracks, then correct and validate the
   nucleus labels under `2_nucleus/`.
4. **Cell segmentation** (full `itasc` app). Turn the cell foreground and
   contour maps plus the tracked nucleus seeds into tracked cell labels under
   `3_cell/`.
5. **Aggregate quantification** (`itasc-aggregate`). Extract contacts, edges,

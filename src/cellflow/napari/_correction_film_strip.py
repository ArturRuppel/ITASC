...
**row-major, left-to-right** in a ``QGraphicsView`` — time runs across a row
and, when a row fills the visible width, it continues at the left of the next
row below (a "line break"). The arrow of time is shown by a faint frame index at
the head of each row plus a ``time → ↓`` hint in the title, so the reading order
survives the wraps without a number under every tile.
...
_ROW_GAP = 8         # gap between wrapped rows (px)
_HEADER_PX = 22      # left gutter on each row for its starting-frame label
_DEFAULT_ROW_WIDTH = 600  # row width used before the view has a real size
...
class TrackFilmStripPanel(QWidget):
    """A track's per-frame crops, laid left-to-right and wrapped into rows."""
    ...
    def _relayout(self) -> None:
        """Place every tile row-major, wrapping when a row fills the view."""
        self._scene.clear()
        self._frame_items = {}
        self._tile_rects = {}
        self._border_item = None
        ...
        row_width = (
            viewport_w if viewport_w >= self._tile_px + _HEADER_PX
            else _DEFAULT_ROW_WIDTH
        )
        x = float(_HEADER_PX)
        y = 0.0
        row_h = 0.0
        ...
        for tile in self._strip.tiles:
            ...
            if not row_first and x + w > row_width:
                y += row_h + _ROW_GAP          # wrap onto the next row
                x = float(_HEADER_PX)
                row_h = 0.0
                row_first = True
            if row_first:
                self._add_row_header(y, tile.frame)
                row_first = False
    ...
    def _add_row_header(self, y: float, frame: int) -> None:
        """A faint starting-frame index in the gutter at the head of a row."""
        text = self._scene.addText(str(int(frame)))
        text.setDefaultTextColor(QColor(160, 160, 160))
        font = text.font()
        font.setPointSizeF(max(6.0, _HEADER_PX - 8))
        text.setPos(-2, y)
...
class _StripView(QGraphicsView):
    ...
    def resizeEvent(self, event) -> None:  # noqa: N802 (Qt override)
        ...
        # Re-wrap so rows track the available width.

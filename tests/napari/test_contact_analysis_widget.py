...
def test_visualize_builds_when_missing_then_shows(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mod, "ensure_contacts", fake_ensure)
...
def test_visualize_uses_existing_h5_without_rebuild(monkeypatch, tmp_path):
    ...
    def boom(*args, **kwargs):
        raise AssertionError("ensure_contacts must not be called when .h5 exists")
    ...
    monkeypatch.setattr(mod, "ensure_contacts", boom)
    monkeypatch.setattr(mod, "read_position_contacts", lambda _p: {"cells": [1]})
    monkeypatch.setattr(mod, "add_contact_analysis_layers", fake_add)
...
def test_revisualize_same_position_skips_layer_rebuild(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mod, "ensure_contacts", lambda **k: (k["output_path"], True))
    monkeypatch.setattr(mod, "read_position_contacts", lambda _p: {"cells": [1]})
    monkeypatch.setattr(mod, "add_contact_analysis_layers", fake_add)
...
def test_recompute_rereads_h5_instead_of_serving_stale_cache(monkeypatch, tmp_path):
    ...
    shown: list = []
    monkeypatch.setattr(mod, "ensure_contacts", fake_ensure)
    monkeypatch.setattr(mod, "read_position_contacts", fake_read)
    monkeypatch.setattr(
...
def test_contact_analysis_widget_forwards_visualizer_options(monkeypatch, tmp_path):
    ...
    add_calls = []
    monkeypatch.setattr(mod, "read_position_contacts", lambda _path: contact_analysis)

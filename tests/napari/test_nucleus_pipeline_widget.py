...
def test_source_auto_preview_rejects_programmatic_conflicting_activation(tmp_path):
    _app, viewer = _make_viewer()
    widget_class = _load_workflow_widget_class()
    widget = widget_class(viewer)
    widget._pos_dir = tmp_path / "pos00"
...
def test_database_browser_activity_blocks_other_viewer_tools(tmp_path):
    _app, viewer = _make_viewer()
    widget_class = _load_workflow_widget_class()
    widget = widget_class(viewer)
    widget._ultrack_db_path = lambda: Path(__file__)
...
def test_correction_activity_blocks_other_viewer_tools(monkeypatch):
    _app, viewer = _make_viewer()
    widget_class = _load_workflow_widget_class()
    widget = widget_class(viewer)
    monkeypatch.setattr(
...
def test_source_auto_preview_still_blocks_tools_after_worker_returns(tmp_path, monkeypatch):
    _app, viewer = _make_viewer()
    widget_class = _load_workflow_widget_class()
    widget = widget_class(viewer)
    pipeline_module = _get_pipeline_module()

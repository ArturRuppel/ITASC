...
Modules are discovered via the ``cellflow.analysis_modules``
entry-point group, or by scanning a plugin folder.

Example third-party module (in ``pyproject.toml``)::

    [project.entry-points."cellflow.analysis_modules"]
    my_module = "my_package:MyModule"

Example usage::

    from cellflow.backend.analysis_modules import discover_modules
...
ENTRY_POINT_GROUP = "cellflow.analysis_modules"
...
def discover_modules(
    ...
    1. Built-in modules in ``cellflow.analysis.builtins``
    2. Entry points in the ``cellflow.analysis_modules`` group
    ...
    for d in plugin_dirs or []:
        ...
        for py_file in sorted(dirpath.glob("*.py")):
            if py_file.name.startswith("_"):
                continue
            module_name = f"cellflow_plugin_{py_file.stem}"
            try:

...
from cellflow.utils.structures import TissueGraphDataset, TissueGraphTimeSeries
...
class ViewerState(QObject):
    ...
    # -- pipeline project (project_dir + schema) ---------------------------

    @property
    def project_dir(self) -> Optional[Path]:
        return self._project_dir

    def set_project_dir(self, path: Optional[Path | str]) -> None:
        """Set the pipeline project root and load the schema from disk if present."""
        if path is None:
            self._project_dir = None
            self._pipeline_schema = None
        else:
            self._project_dir = Path(path)
            schema_file = self._project_dir / "pipeline_schema.json"
            if schema_file.exists():
                try:
                    from cellflow.core.schema import PipelineSchema
                    self._pipeline_schema = PipelineSchema.load(schema_file)
                except Exception:
                    self._pipeline_schema = None
            else:
                self._pipeline_schema = None
    ...
    @property
    def pipeline_schema(self) -> "Optional[PipelineSchema]":
        return self._pipeline_schema

    @pipeline_schema.setter

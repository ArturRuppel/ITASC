...
def merge_cells(
    ...
    *,
    ...
    After merging, cleans up any stranded pixels left by the morphological
    closing operation.
    """
    ...
    # Merge any remaining pixels of la into lb (handles stranded pixels and
    # components that didn't get included in the closing operation)
    remaining_la = seg == la
    n_remaining = int(np.sum(remaining_la))
    if n_remaining > 0:
        seg[remaining_la] = lb
        log.debug("merge_cells: reassigned %d remaining pixels of %s to %s",
                  n_remaining, la, lb)

    # Clean up any stranded pixels that may have been created during the process

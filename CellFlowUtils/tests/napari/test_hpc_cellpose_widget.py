...
def _load_module(monkeypatch):
    ...
    sys.modules.pop("cellflow_utils.napari.hpc_cellpose_widget", None)
    return importlib.import_module("cellflow_utils.napari.hpc_cellpose_widget")
...
def test_widget_constructor_supports_napari_viewer_injection(monkeypatch):
    mod = _load_module(monkeypatch)
    from napari._qt._qplugins._qnpe2 import _get_widget_viewer_param

    signature = inspect.signature(mod.HpcCellposeWidget.__init__)

    assert "napari_viewer" in signature.parameters
    assert (
        _get_widget_viewer_param(
            mod.HpcCellposeWidget,
            "CellFlow Utils: HPC Cellpose",
        )
        == "napari_viewer"
    )

...
from cellflow_utils.core.data_prep import DatasetConfig, discover_metadata, run as run_prep
...
class DataPrepWidget(QWidget):
    ...
    def __init__(self, viewer: napari.Viewer, main_widget: CellFlowMainWidget, parent: QWidget | None = None) -> None:
        ...
        # Z downsample
        z_ds_row = QHBoxLayout()
        z_ds_row.addWidget(QLabel("Z Downsample:"))
        self.z_ds_spin = QSpinBox()
        self.z_ds_spin.setRange(1, 16)
        self.z_ds_spin.setValue(1)
        z_ds_row.addWidget(self.z_ds_spin)
        layout.addLayout(z_ds_row)

        # Channel mapping
        channel_row = QHBoxLayout()
        channel_row.addWidget(QLabel("Nucleus:"))
        self.nucleus_channel_combo = QComboBox()
        channel_row.addWidget(self.nucleus_channel_combo, 1)
        channel_row.addWidget(QLabel("Cell:"))
        self.cell_channel_combo = QComboBox()
        channel_row.addWidget(self.cell_channel_combo, 1)
        channel_row.addWidget(QLabel("NLS:"))
        self.nls_channel_combo = QComboBox()
        channel_row.addWidget(self.nls_channel_combo, 1)
        layout.addLayout(channel_row)
        self._set_all_channel_options(["CSUTRANS", "CSU488", "CSU561", "CSU642"])
    ...
    _CHANNEL_DEFAULTS = {"cell": 1, "nls": 2, "nucleus": 3}

    def _channel_combo(self, channel_key: str) -> QComboBox:
        return getattr(self, f"{channel_key}_channel_combo")

    def _set_all_channel_options(self, channel_names: list[str]) -> None:
        for channel_key in ("nucleus", "cell", "nls"):
            self._set_channel_options(channel_key, channel_names)

    def _set_channel_options(
        self,
        channel_key: str,
        ...
        *,
        ...
        combo = self._channel_combo(channel_key)
        current = self._channel_value(channel_key) if selected is None else int(selected)
        ...
        if channel_names:
            ...
            combo.addItem(f"{current}: Channel {current}", current)
        combo.blockSignals(False)
        self._select_channel(channel_key, current)

    def _select_channel(self, channel_key: str, channel: int) -> None:
        combo = self._channel_combo(channel_key)
        idx = combo.findData(int(channel))
        if idx < 0:
            combo.addItem(f"{int(channel)}: Channel {int(channel)}", int(channel))
            idx = combo.findData(int(channel))
        combo.setCurrentIndex(idx)

    def _channel_value(self, channel_key: str) -> int:
        combo = self._channel_combo(channel_key)
        data = combo.currentData()
        if data is None:
            return self._CHANNEL_DEFAULTS[channel_key]
        return int(data)

    def _set_nucleus_channel_options(
        self, channel_names: list[str], *, selected: int | None = None
    ) -> None:
        self._set_channel_options("nucleus", channel_names, selected=selected)

    def _select_nucleus_channel(self, channel: int) -> None:
        self._select_channel("nucleus", channel)

    def _nucleus_channel_value(self) -> int:
        return self._channel_value("nucleus")
    ...
    def _on_metadata_returned(self, result: dict) -> None:
        ...
        channel_names = result.get("channel_names") or []
        ...
        self._set_all_channel_options(channel_names)
    ...
    def _on_run(self) -> None:
        ...
        config = DatasetConfig(
            ...
            z_downsample=self.z_ds_spin.value(),
            frame_start=self.frame_start_spin.value(),
            frame_end=self.frame_end_spin.value(),
            cell_channel=self._channel_value("cell"),
            nls_channel=self._channel_value("nls"),
            nucleus_channel=self._channel_value("nucleus"),
        )
    ...
    def _on_run_in_terminal(self) -> None:
        ...
        python_code = (
            "from cellflow_utils.core.data_prep import DatasetConfig, run\n"
            f"config = DatasetConfig(ndtiff_path={ndtiff_path!r}, root_dir={root_dir!r}, "
            f"positions=[{positions}], xy_downsample={ds}, z_downsample={z_ds}, "
            f"frame_start={frame_start}, frame_end={frame_end}, "
            f"cell_channel={cell_channel}, nls_channel={nls_channel}, "
            f"nucleus_channel={nucleus_channel})\n"
    ...
    def set_state(self, state: dict) -> None:
        ...
        if "z_downsample" in state:
            self.z_ds_spin.setValue(state["z_downsample"])
        if "cell_channel" in state:
            self._select_channel("cell", state["cell_channel"])
        if "nls_channel" in state:
            self._select_channel("nls", state["nls_channel"])
        if "nucleus_channel" in state:
            self._select_channel("nucleus", state["nucleus_channel"])
